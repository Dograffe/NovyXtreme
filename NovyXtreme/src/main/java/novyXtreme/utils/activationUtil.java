...
public class activationUtil
{
    public static void activateGate(Stargate stargate, Player player)
    {
        if(!stargate.setActive(true))
        {
            ...
            BukkitTask gateTimeout = new stargateTimeout(NovyXtreme.getPlugin(), stargate, player).runTaskLater(NovyXtreme.getPlugin(), 1200L);
    ...
    public static void deactivateGate(Stargate stargate, @Nullable Player player)
    {
        ...
        if(player != null)
        {
            player.removeMetadata("StargateActive", NovyXtreme.getPlugin());
        }
    }
    public static void nxcompleteStart(Block leverblock, Player player)
    {
        ...
        BukkitTask gateTimeout = new nxCompleteTimeout(NovyXtreme.getPlugin(), player).runTaskLater(NovyXtreme.getPlugin(), 1200L);
    }
